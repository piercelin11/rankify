...
**功能說明**:
- 對藝術家的專輯、EP、單曲進行排序
- 訪客模式：進度存至 localStorage，完成後引導登入遷移
- 登入模式：支援草稿自動儲存與恢復
- 提供過濾與篩選功能
...
- 對專輯內的歌曲進行排序
- 訪客模式：進度存至 localStorage，完成後引導登入遷移
- 登入模式：自動建立 submission 記錄，支援草稿儲存與恢復
...
**路徑**: `/ranking/public/[submissionId]`
**權限**: 🔓 公開存取
**動態參數**: `submissionId` - 提交記錄 ID
